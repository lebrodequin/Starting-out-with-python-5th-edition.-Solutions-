...
while keep_going.lower() == 'y':
    key = random.choice(list(capital_dict.keys()))
    value = input(f'what is the capital of {key}: ')
    if value == capital_dict.get(key):
        print('It is correct!')
        correct_answers += 1
    else:
        print(f'It is incorrect, the correct answer is {capital_dict.get(key)} ')
        incorrect_answers += 1
    keep_going = input('would you like to continue quizz? (y/n): ')

print(f'the number of correct answers id {correct_answers}\n'
      f'the number of incorrect answers is {incorrect_answers}')